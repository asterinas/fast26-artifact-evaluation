...
run_benchmark() {
    ...
    "${YCSB_BIN}" load sqlite -P "${WORKLOAD_DIR}/${workload}" -p recordcount=${RECORD_COUNT} -p operationcount=${OPERATION_COUNT} -p sqlite.db="${db_file}"
    ...
    local output=$(mktemp)
    "${YCSB_BIN}" run sqlite -P "${WORKLOAD_DIR}/${workload}" -p recordcount=${RECORD_COUNT} -p operationcount=${OPERATION_COUNT} -p sqlite.db="${db_file}" 2>&1 | tee "${output}"
