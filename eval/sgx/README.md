...
- Run:
  ...
  ./reproduce.sh
  ```
- Output: `results/reproduce_result.json`, per-test logs; plot via:
  ```bash
...
- Prep: If traces are missing, download from [MSR-Trace](https://iotta.snia.org/traces/block-io/388), unzip them, and place all trace files under `msr-test/`.
- Run all:
  ```bash
  cd trace
  ./reproduce.sh
  ```
- Output: `results/trace_reproduce_result.json` plus per-trace logs; plot with:
  ```bash
  python3 plot_result.py
  ```
...
- Run all:
  ...
  ./reproduce.sh
  ```
- Output: logs/JSON under `results/`; plot with:
  ```bash
  python3 plot_result.py
  ```
...
- Run:
  ...
  ./reproduce.sh
  ```
- Output: `results/cache_size_result.json` and raw FIO logs; plot with:
  ```bash
  python3 plot_result.py
  ```
...
- Run:
  ...
  ./reproduce.sh
  ```
- Output: `results/reproduce_results.csv`; plot via:
  ```bash
  python3 plot_result.py
  ```
...
- Run:
  ...
  ./reproduce.sh
  ```
- Output: logs in `results/`; plot via:
  ```bash
  python3 plot_result.py
  ```
...
- Run:
  ...
  ./reproduce.sh
  ```
- Output: `results/optimization_results.json` plus logs; plot with:
  ```bash
  python3 plot_result.py
  ```
...
- Run (needs `../fio` built):
  ...
  ./reproduce.sh
  ```
- Output: logs in `results/`; visualize stacks with:
  ```bash
  python3 plot_result.py
  ```
