...
def plot_disk_comparison(ax, results, disk_types, colors, hatches, title):
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.0),
              ncol=3, frameon=False, fontsize=13, handletextpad=0.3, columnspacing=0.8)
...
def plot_grouped_bars(results, out_path: Path):
    ...
    # Match colors and hatches from image:
    # CryptDisk: Red/Vertical, PfsDisk: Green/Horizontal, StrataDisk: Blue/Diagonal
    disk_types = ["CryptDisk", "PfsDisk", "StrataDisk"]
    colors = ["#e74c3c", "#8eb060", "#4a90e2"] 
    hatches = ["||||", "----", "////"]

    plot_disk_comparison(ax1, write_res, disk_types, colors, hatches, "(a) Writes in SGX")
    plot_disk_comparison(ax2, read_res, disk_types, colors, hatches, "(b) Reads in SGX")
...
def main():
    ...
    parser.add_argument("--input", default="results/reproduce_result.json", type=Path)
