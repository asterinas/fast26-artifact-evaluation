...
cd occlum_instance
TCS_NUM=$(($(nproc) * 2))
new_json="$(jq --argjson THREAD_NUM ${TCS_NUM} '.resource_limits.user_space_size="1500MB" |
    ...
    .mount += [{"target": "/ext2", "type": "ext2", "options": {"disk_size": "60GB","disk_name":"cryptdisk"}}]' Occlum.json)" && \
